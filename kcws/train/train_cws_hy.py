...
class Model:
    def __init__(self, embeddingSize, distinctTagNum, c2vPath, numHidden):
        ...
        if use_idcnn:
            self.model = IdCNN(layers, 3, num_hidden, embedding_size,
                               max_sentence_len, num_tags)
        else:
            self.model = BiLSTM(
                num_hidden, max_sentence_len, num_tags)
...
def cws_train(trainfile, testfile, vecfile, logdir='logs', numHidden=100, 
        batchSize=100, trainSteps=150000, trackHistory=6, maxSentenceLen=80, 
        embeddingSize=50, numTags=4, learningRate=0.001, useIdcnn=True):
    ...
    with graph.as_default():
        model = Model(embedding_size, num_tags,
                      vecfile, num_hidden)
        print("train data path:", trainDataPath)
        X, Y = inputs(trainDataPath)
        tX, tY = do_load_data(testfile)
                ...
                try:
                    ...
                    if (step + 1) % 1000 == 0 or step == 0:
                        ...
                        if acc > bestAcc:
                            if step:
                                sv.saver.save(
                                    sess, logdir + '/best_model')
                            bestAcc = acc
                            trackHist = 0
                        elif trackHist > trackHistory:
                            print(
